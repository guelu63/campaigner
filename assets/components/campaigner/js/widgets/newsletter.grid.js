Campaigner.grid.Newsletter = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,fields: ['id', 'docid', 'state', 'sent_date', 'auto', 'nl_count', 'total', 'sent', 'bounced', 'sender', 'sender_email', 'subject', 'date', 'groups', 'priority', 'subscribers']
        ,paging: true
        // ,grouping: true
        // ,groupBy: 'auto'
        ...
        ,{
            ...
            header: _('campaigner.newsletter.subscribers')
        ...
        tbar : [{
            ...
            ,text: _('campaigner.newsletter.toggleauto')
            ,listeners: {
              'click': {fn: this.toggleAuto, scope: this}
            }
        }, '->', {
            xtype: 'splitbutton'
            ,text: _('campaigner.newsletter.add')
...
Ext.extend(Campaigner.grid.Newsletter,MODx.grid.Grid,{
    activate: function() {
        // Ext.getCmp('card-container').getLayout().setActiveItem('campaigner-panel-newsletter-base');
    }
    ...
    ,_renderNewsletter: function(value, p, rec) {
        return '<a href="?a=30&id='+ rec.data.docid +'&letter=1">'+ value +'</a>';
    }
    ...
    ,editNewsletter: function() {
        ...
        // this.config.newsletter = cs;
        var edit_panel = Ext.getCmp('campaigner-panel-newsletter-editarticles');
        edit_panel.config.newsletter = cs;
        edit_panel.activate();
    }
    ...
    ,editArticles: function(btn, e) {
        ...
        // var w = MODx.load({
        //     xtype: 'campaigner-window-newsletter-editarticles'
        //     ,params: {
        //         docid: this.menu.record.docid
        //     }
        //     ,listeners: {
        //         'success': {fn:this.refresh,scope:this}
        //     }
        ...
        // w.setValues(this.menu.record);
        // w.show(e);
        // var grid_articles = Ext.getCmp('campaigner-newsletter-grid-articles');
        // grid_articles.store.load({params:{nlid: this.menu.record.id || 0, docid: this.menu.record.docid || 0}});
        // return;
    }
    ,getMenu: function() {
        var m = [];
        if (this.getSelectionModel().getCount() == 1) {
            ...
            // }
            // if(MODx.perm.newsletter_editarticles) {
            //     m.push({
            //         text: _('campaigner.newsletter.editarticles')
            //         ,handler: this.editArticles
            //     });
            // }
...
Campaigner.combo.TestMails = function(config) {
    config = config || {};
    Ext.applyIf(config,{
      fieldLabel: _('campaigner.newsletter.sendtest.email')
      ,name: 'email'
      ,id: this.ident+'-email'
      ,store: MODx['config']['campaigner.test_mail'].split(',')
      ,emptyText: _('campaigner.newsletter.sendtest.email')
    });
    Campaigner.combo.TestMails.superclass.constructor.call(this,config);
}
Ext.extend(Campaigner.combo.TestMails,MODx.combo.ComboBox);
Ext.reg('campaigner-combo-testmails', Campaigner.combo.TestMails);
...
Campaigner.window.NewsletterTest = function(config) {
    ...
    Ext.applyIf(config,{
      ...
      ,fields: [{
        ...
        xtype: 'campaigner-combo-testmails'
        ,anchor: '100%'
        ,fieldLabel: _('campaigner.newsletter.sendtest.email')
        ,name: 'email'
...
Campaigner.window.NewsletterPreview = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        //             this.setHeight(this.maxHeight) * 0.8;
        //         }
        //         this.center();
        //     }, scope: this}
        // }
